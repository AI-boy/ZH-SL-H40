...
float load_cc_I=33;						//LED灯工作电流  负载设定值:0.1--1.2A(2--120)
...
void read_param_eeprom(void)
{
	...
	if(volt_id==0x01)			//9--12.3V  磷酸铁锂电池
	{
		...
		discharge_i_baohu = 4.50*160.0;				//放电电流保护点  过流保护  4.5A
		
		discharge_i_limit = 4.00*160.0;				//放电电流限定值 4.0A
	...
	else
	{
		...
		light_on_pv_level = (5.0/61)*1000;		   //亮灯光伏电压门限值
