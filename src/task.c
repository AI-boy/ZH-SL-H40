...
//			
		...
		if(handle_an1_process_flag==0x01)
		{
			...
			if((temperature_value>4)&&(temperature_value<602))  	//正常工作温度范围：-30℃---70℃
			{
				temper_time_count++;
		...
		if(handle_an2_process_flag==0x01)
		{
			...
			pv_voltage_value=start_ad_convert(AN2_CHANNEL);  		//采样通道选择	AN2		光伏板电压    分压电阻缩减比例 1:61 
			...
			if((pv_voltage_value>=light_off_pv_level)&&(pv_voltage_value<1000)) 	//10V---60V  光伏板电压范围
			{																			
				pv_time_count++;
		...
		if(handle_an3_process_flag==0x01)
		{
			...
			if(((battery_voltage_value<(dianchi_guoya_baohu+volt_id*5))||(sys_power_ch==0x01))&&(charge_index==0x00))					//锂电池电压是否正常？  <14.0V
			{
				bt_time_count++;
			...
			else if((battery_voltage_value>=(dianchi_guoya_baohu+volt_id*5))&&(charge_index<=0x01)&&(sys_power_ch==0x00))
			{		
					bt_led_status=0x00;
		...
		//------------------------------------------------------------------------------------------------------------------输出电流
		if(handle_an10_process_flag==0x01)  												//电流检测范围：
		{
			...
			if(system_status ==	SYSTEM_DISCHARGE_STATUS)									//系统处于放电状态
			{
				...
				{ 									
					...
					if(over_load_time_count>1000)												//过载延时保护
					{										
						discharge_index=0x03;
				...
				else if((battery_voltage_value<=dianchi_qianya_huifu)&&(discharge_index==0x01))
				{
				 ...
				 if(((battery_voltage_value>=dianchi_qianya_baohu)||(sys_power_ch==0x01))&&(discharge_index==0x02))    		//电池电压>欠压保护点
				 {
					...
					{								    
						...
						//---------------------------------------------------------------------------限定功率输出
						if( ((load_cc_I/100)*160) > (Load_Pmax*160/(load_voltage_value*61/1000)) )	
						{
							PID_CC_discharge.SetPoint=(Load_Pmax*160)/(load_voltage_value*61/1000);
						...
						{
							PID_CC_discharge.SetPoint=(load_cc_I/100)*160;
				 ...
				 else if((battery_voltage_value<dianchi_qianya_baohu)&&(discharge_index==0x02)) //   欠压保护 
				 {
						...
						if(battery_low_time_count>2000)
						{													
							discharge_index=0x03;
	...
	switch(system_status)
	{
		case SYSTEM_NOP_STATUS:      
									if(nop_index==0x00)
									{
										...
										PID_CV_charge.SetPoint   = dianchi_cv_level+1;   // 设定目标 Desired value V
										...
										PID_CC_discharge.SetPoint   = (load_cc_I/100)*160;    	// 设定目标 Desired value 330mA  
		...
		case SYSTEM_CHARGE_STATUS:	
										if(charge_index==0x01)
										{
										    ...
										    PID_CV_charge.SetPoint   = dianchi_cv_level+1;    		// 设定目标 Desired value    --恒压设定
		...
		case SYSTEM_DISCHARGE_STATUS:
										...
										if(discharge_index==0)
										{
										    ...
										    PID_CC_discharge.SetPoint   = (load_cc_I/100)*160;   	// 设定目标 Desired value 默认330mA  
...
void Mppt_Charge(void)
{
					...
					else if((battery_voltage_value>=dianchi_cv_level)&&(battery_voltage_value<dianchi_guoya_baohu)&&(charge_index==0x03)) //CV恒压充电  13.85
					{
							cv_level_time_count++;
					...
					else if( (battery_voltage_value<dianchi_guoya_baohu) && (charge_index==0x04) ) //CV恒压充电  13.85
					{
							pid_index = 0x02;
